...
erDiagram
    ...
    REVENUE_SOURCES ||--o{ ORDER_COMMERCIAL_PROFILES : classifies
    ORDERS ||--o| ORDER_COMMERCIAL_PROFILES : configures
    ORDERS ||--o{ COMMERCIAL_QUOTATIONS : prices
    COMMERCIAL_QUOTATIONS ||--o| COMMERCIAL_CONTRACTS : supports
    ORDER_ACCEPTANCES ||--o| ACCEPTANCE_CERTIFICATES : documents
    ORDERS ||--o{ COMMERCIAL_PAYMENTS : collects
    ORDERS ||--o{ COMMERCIAL_EXPENSES : costs
    ORDERS ||--o{ COMMERCIAL_DOCUMENTS : stores
```
...
| Commercial       | `revenue_sources`, `order_commercial_profiles`, `commercial_quotations`, `commercial_contracts`, `acceptance_certificates`, `commercial_payments`, `commercial_expenses`, `commercial_documents` | whole-UZS exact amounts; one active quote/contract; accepted-price overpayment guard in transaction; immutable checksummed document snapshots           |
...
The idempotent seed creates one `DEMO` service area; Plumbing, Electrical, Repair, and Landscaping categories; their version-one quality checklists; six request sources; five revenue-source classes; all declared permissions; and Resident, Operator/Manager, Executor, and Administrator roles. Electrical inspection is required. It creates no fake residents, staff accounts, executor profiles, orders or commercial transactions.
